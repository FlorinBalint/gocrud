// Code generated by handlersgen. DO NOT EDIT.

package handlers

import (
	"context"
	"database/sql"
	testdata "github.com/FlorinBalint/gocrud/entitygen/testdata"
	gocrudv1 "github.com/FlorinBalint/gocrud/proto/v1"
	"github.com/FlorinBalint/gocrud/sqldialect"
	"google.golang.org/genproto/googleapis/type/date"
	"google.golang.org/grpc/codes"
	"google.golang.org/grpc/status"
	"google.golang.org/protobuf/types/known/timestamppb"
	"time"
)

// AutogenUserUpsertHandler handles Upsert requests for AutogenUser entities.
type AutogenUserUpsertHandler struct {
	db      DBExecutor
	dialect sqldialect.Builder
}

// NewAutogenUserUpsertHandler creates a new handler for upserting AutogenUser entities.
func NewAutogenUserUpsertHandler(db DBExecutor, backend sqldialect.BackendType) (*AutogenUserUpsertHandler, error) {
	b, err := sqldialect.ForBackend(backend)
	if err != nil {
		return nil, err
	}
	return &AutogenUserUpsertHandler{db: db, dialect: b}, nil
}

// Upsert handles an UpsertAutogenUserRequest and returns the upserted AutogenUser.
func (h *AutogenUserUpsertHandler) Upsert(ctx context.Context, req *testdata.UpsertAutogenUserRequest) (*testdata.AutogenUser, error) {
	entity := req.GetAutogenUser()
	if entity == nil {
		return nil, status.Errorf(codes.InvalidArgument, "missing AutogenUser in request")
	}

	// Validate all primary key fields are populated.
	if entity.GetId_1() == "" {
		return nil, status.Errorf(codes.InvalidArgument, "missing required primary key field: id_1")
	}
	if entity.GetId_2() == 0 {
		return nil, status.Errorf(codes.InvalidArgument, "missing required primary key field: id_2")
	}

	// Build INSERT values for non-auto-generated fields.
	columns := []string{"id_1", "id_2", "email"}
	values := []*gocrudv1.Value{
		stringValue(entity.GetId_1()),
		intValue(entity.GetId_2()),
		stringValue(entity.GetEmail()),
	}
	conflictCols := []string{"id_1", "id_2"}
	var returningCols []string
	if h.dialect.SupportsReturning() {
		returningCols = []string{"id_3", "commit_timestamp", "date_of_birth"}
	}
	q := sqldialect.NewUpsertQuery("composite_entities", columns, values, conflictCols, returningCols)
	sqlStr, args, err := h.dialect.BuildUpsert(q)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "building upsert SQL: %v", err)
	}
	var autoGen0 int64
	var autoGen1 time.Time
	var autoGen2 time.Time
	err = execInsertAndScan(ctx, h.db, h.dialect.SupportsReturning(),
		sqlStr, args,
		func(lastInsertId int64) (string, []any, error) {
			if lastInsertId > 0 {
				entity.Id_3 = lastInsertId
			}
			selectCols := []string{"id_3", "commit_timestamp", "date_of_birth"}
			filter := h.primaryKeyFilter(entity)
			selQ := sqldialect.NewSelectQuery("composite_entities", selectCols, filter, nil, 1, 0, false)
			return h.dialect.BuildSelect(selQ)
		},
		func(row RowScanner) error {
			return row.Scan(&autoGen0, &autoGen1, &autoGen2)
		},
	)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, status.Errorf(codes.Internal, "upsert did not return any rows")
		}
		return nil, status.Errorf(codes.Internal, "executing upsert: %v", err)
	}
	// Set auto-generated values on the entity.
	entity.Id_3 = autoGen0
	entity.CommitTimestamp = timestamppb.New(autoGen1)
	entity.DateOfBirth = &date.Date{Year: int32(autoGen2.Year()), Month: int32(autoGen2.Month()), Day: int32(autoGen2.Day())}
	return entity, nil
}

// primaryKeyFilter builds a WHERE filter matching all primary key fields.
func (h *AutogenUserUpsertHandler) primaryKeyFilter(entity *testdata.AutogenUser) *gocrudv1.Filter {
	return &gocrudv1.Filter{
		Filter: &gocrudv1.Filter_Composite{
			Composite: &gocrudv1.CompositeFilter{
				Op: gocrudv1.CompositeFilter_AND,
				Filters: []*gocrudv1.Filter{
					{
						Filter: &gocrudv1.Filter_Condition{
							Condition: &gocrudv1.Condition{
								Column:  "id_1",
								Op:      gocrudv1.Operator_EQUAL,
								Operand: &gocrudv1.Condition_Value{Value: stringValue(entity.GetId_1())},
							},
						},
					},
					{
						Filter: &gocrudv1.Filter_Condition{
							Condition: &gocrudv1.Condition{
								Column:  "id_2",
								Op:      gocrudv1.Operator_EQUAL,
								Operand: &gocrudv1.Condition_Value{Value: intValue(entity.GetId_2())},
							},
						},
					},
					{
						Filter: &gocrudv1.Filter_Condition{
							Condition: &gocrudv1.Condition{
								Column:  "id_3",
								Op:      gocrudv1.Operator_EQUAL,
								Operand: &gocrudv1.Condition_Value{Value: intValue(entity.GetId_3())},
							},
						},
					},
				},
			},
		},
	}
}
