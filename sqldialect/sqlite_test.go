...
func TestSQLiteBuildInsert(t *testing.T) {
	tests := []struct {
		...
		// RETURNING clause
		{
			...
			wantSQL:  `INSERT INTO "users" ("name", "email") VALUES (?, ?) RETURNING "id"`,
		...
		},
		{
			...
			wantSQL:  `INSERT INTO "users" ("id", "name") VALUES (DEFAULT, ?) RETURNING "id"`,
			wantArgs: []any{"carol"},
		},
		{
...
func TestSQLiteRejectsIntervalValue(t *testing.T) {
	...
	t.Run("in SELECT WHERE clause", func(t *testing.T) {
		q := selectQuery{
			table:  "sessions",
	...
	t.Run("in INSERT", func(t *testing.T) {
		q := insertQuery{
			table:   "sessions",
	...
	t.Run("in UPDATE SET", func(t *testing.T) {
		q := updateQuery{
			table: "sessions",
			updates: []*gocrudv1.ColumnUpdate{
				{Column: "duration", Assignment: &gocrudv1.ColumnUpdate_Value{Value: interval}},
			},
	...
	t.Run("in UPSERT", func(t *testing.T) {
		q := upsertQuery{
			table:        "sessions",
